...
int eval (int v1, char operation, int v2) {
    if (operation == '+') {
        return v1 + v2;
    }
    else if (operation == '-') {
...
void popAndEval(valueStack *val, operatorStack *operation){
  int v1, v2, v3;
  char op;
  op = (char) operation->opGetTop();
  operation->opPop ();
  v2 = val->valGetTop ();
  val->valPop ();
  v1 = val->valGetTop ();
...
void processExpression (Token inputToken, TokenReader *tr)
{
    ...
    if (inputToken.equalsType(VALUE))
      {
       ...
       vStack.valPush(inputToken.getValue());
      }
    ...
    /* The expression contains an OPERATOR */
    else if (inputToken.equalsType(OPERATOR))
      {
       ...
       if(inputToken.getOperator() == '('){
            opStack.opPush((int)inputToken.getOperator());
       }
       else if(inputToken.getOperator() == '+' || inputToken.getOperator() == '-' ){
            while(opStack.opIsEmpty() == false && (opStack.opGetTop() == '+' || opStack.opGetTop() == '-' || opStack.opGetTop() == '*' || opStack.opGetTop() == '/' )){
              popAndEval(&vStack, &opStack);
            }
            opStack.opPush((int)inputToken.getOperator());
        }
       else if(inputToken.getOperator() == '*' || inputToken.getOperator() == '/' ){
            while(opStack.opIsEmpty() == false && (opStack.opGetTop() == '*' || opStack.opGetTop() == '/' )){
                popAndEval(&vStack, &opStack);
            }
            opStack.opPush((int)inputToken.getOperator());
        }
        else if(inputToken.getOperator() == ')'){
            while(opStack.opIsEmpty() == false && opStack.opGetTop() != '('){
              popAndEval(&vStack, &opStack);
            }
             if(opStack.opIsEmpty() == true){
              cout << "error" << endl;
            }
            else{
              opStack.opPop();
 ...
 // add code to perform this operation here
  ...
  while (opStack.opIsEmpty() == false ) {
     // pop and eval
     popAndEval(&vStack, &opStack);
    }
    cout << "Top of value stack is " << vStack.valGetTop() << endl;   
  }
